    ...
    for square in opponent_squares:
        ...
        for key, value in game_matrix_dict.items():
            ...
            if (
                ...
                # check that the square is not a computer square or an opponent square and not already in the opponent_adjacent_squares list
                if (
                    game_matrix_dict[key][0] not in computer_squares
                    and game_matrix_dict[key][0] not in opponent_squares
                    and game_matrix_dict[key][0] not in opponent_adjacent_squares
                ):
                    opponent_adjacent_squares.append(game_matrix_dict[key][0])
...
def hard_move(player: str):
    ...
    # -- list to store the opponent squares and the player squares
    opponent_squares = []
    player_squares = []

    # -- fill the opponent_squares and player_squares lists
    for key, value in game_matrix_dict.items():
        if game_matrix_dict[key][2] != player and game_matrix_dict[key][1] == True:
            opponent_squares.append(game_matrix_dict[key][0])
        if game_matrix_dict[key][2] == player:
    ...
    if len(player_squares) >= 2:
        ...
        print(f"Player Has Two Adjacent Squares - The Open Square is: {has_two}")
        if has_two:
    ...
    if len(opponent_squares) >= 2:
        ...
        print(f"Opponent Has Two Squares Adjacent - The Open Square is: {has_two}")
        if has_two:
            # set the square selection for the blocking move
            set_selection(has_two[0][0], has_two[0][1], player)
            print(f"Game Matrix: {game_matrix_dict}")
            return

    # --check that there are empty adjacent squares to the player squares
    if len(player_squares) >= 1:
        # get the empty squares that are adjacent to the player squares
        player_adjacent_squares = []
        # get a list of squares that are adjacent to the player_squares
            ...
            for key, value in game_matrix_dict.items():
                ...
                if (
                    ...
                    if (
                        ...
                        and game_matrix_dict[key][0] not in opponent_squares
                        and game_matrix_dict[key][0] not in player_adjacent_squares
                    ):
                        player_adjacent_squares.append(game_matrix_dict[key][0])

        print()
        print(f"Player Adjacent Squares: {player_adjacent_squares}")
        print()

        # Now make a choice of the adjacent squares
        player_adj_choice = choice(player_adjacent_squares)
        print(f"Oppenent Adjacent Squares Choice: {player_adj_choice}")
        print()

        # set the square selection to the adj_choice
        set_selection(player_adj_choice[0], player_adj_choice[1], player)

        print(f"Game Matrix: {game_matrix_dict}")
        return

    # --if there are no empty squares adjacent to the player squares, then check for empty squares adjacent to the opponent squares
    opponent_adjacent_squares = []
    # get a list of squares that are adjacent to the opponent_squares
    for square in opponent_squares:
        # print(f"SQUARE: {square}")
        ...
        for key, value in game_matrix_dict.items():
            ...
            if (
                ...
                or game_matrix_dict[key][0]
                == (square[0] - 1, square[1] + 1)  # top right
                or game_matrix_dict[key][0] == (square[0] + 1, square[1] - 1)
            ):  # bottom left
    ...
    print()
    print(f"Opponent Adjacent Squares: {opponent_adjacent_squares}")
    print()

    # Now make a choice of the adjacent squares
    opponent_adj_choice = choice(opponent_adjacent_squares)
    print(f"Oppenent Adjacent Squares Choice: {opponent_adj_choice}")
    print()

    # set the square selection to the adj_choice
    set_selection(opponent_adj_choice[0], opponent_adj_choice[1], player)
